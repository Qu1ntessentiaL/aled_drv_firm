...
bool AT24xx_Init(void) {
    GPIO_InitTypeDef GPIO_InitStruct = {0};

    __HAL_RCC_GPIOB_CLK_ENABLE();

    GPIO_InitStruct.Pin = WP_PIN; //WP_PIN = write protect pin
...
#else
        HAL_I2C_Mem_Write(&AT24CXX_I2C, AT24CXX_I2C_ADDR, i, I2C_MEMADD_SIZE_16BIT, (uint8_t *) data_buff,
                          AT24CXX_PAGE_BYTE, HAL_MAX_DELAY);
#endif
...
uint16_t AT24Cxx_write(uint16_t addMem_write, uint8_t *data_write, uint16_t size_write) {
    ...
    if ((addMem_write + size_write) < AT24CXX_MAX_MEM_ADDRESS) {
        ...
        uint16_t byte_count_write = AT24CXX_PAGE_BYTE - (addMem_write - (page_count_write *
                                                                         AT24CXX_PAGE_BYTE)); // узнаем сколько байт нужно отправить до следуещей страницы
...
#else
            HAL_I2C_Mem_Write(&AT24CXX_I2C, AT24CXX_I2C_ADDR, addMem_write, I2C_MEMADD_SIZE_16BIT,
                              (uint8_t *) data_buff, size_write, HAL_MAX_DELAY);
#endif
...
#else
        HAL_I2C_Mem_Read(&AT24CXX_I2C, AT24CXX_I2C_ADDR, addMem_read, I2C_MEMADD_SIZE_16BIT, (uint8_t *) data_read,
                         size_read, HAL_MAX_DELAY);
#endif
...
bool AT24Cxx_write_data(uint16_t addMem_write, uint8_t *data, uint8_t len) {
    ...
    if ((import_data == data_check) &&
        (CRC32 == CRC_check)) {  //Если отправленные данные равны принятым, то запись прошла успешно
        return true;
    } else {
        return false;
    }
...
bool AT24Cxx_read_data(uint16_t addMem_read, uint8_t *data, uint8_t len) {
    ...
    if (HAL_CRC_Calculate(&AT24CXX_CRC, (uint32_t *) &import_data, 1) ==
        CRC_check) {  //Если CRC принятых данных равна CRC, которые были записаны - значит данные не битые!
        return true;
    } else {
        return false;
    }
